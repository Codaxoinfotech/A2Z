...
<div class="app-content content ">
  ...
  <div class="row invoice-add">
          ...
          <div class="row row-bill-to invoice-spacing">
            ...
            <h6 class="mb-2">Service Details:</h6>
              <table>
                <tbody>
                <tr>
                    <td class="pe-1"> <strong> Apply No : {{ $data->apply_no }}</strong></td>
                  </tr>
                  <tr>
                    <td class="pe-1"> <strong> Category : {{ $data->category->name }}</strong></td>
                  </tr>
                  <tr>
                    <td class="pe-1"> <strong> Sub Category : {{ $data->sub_category->name }} </strong></td>
                  </tr>
                  <tr>
            ...
            <div class="col-xl-6 p-0 ps-xl-2 mt-xl-0 mt-2">
              ...
              <table>
                <tbody>
                  ...
                  <tr>
                    <td class="pe-1">Proccess Date & Time:</td>
                    <td><strong>{{ $data->proccess_date }} {{ $data->proccess_time }}</strong></td>
                  </tr>

                  <tr>
                    <td class="pe-1">Service Finish Date & Time:</td>
                    <td><strong>{{ $data->work_finish_date }} {{ $data->work_finish_time }}</strong></td>
                  </tr>

                  <tr>
                    <td class="pe-1">Completed Finish:</td>
                    <td><strong>{{ $data->finish_date }} {{ $data->finish_date }}</strong></td>
                  </tr>
          ...
          <form class="source-item">
                  ...
                  <div class="col-12 d-flex product-details-border position-relative pe-0">
                    <div class="row w-100 pe-lg-0 pe-1 py-2">
                      <div class="col-lg-5 col-12 mb-lg-0 mb-2 mt-lg-0 mt-2">
                        <p class="card-text col-title mb-md-50 mb-0">Assign Task to</p>
                            <select class="form-select item-details" id="emp_id">
                            <option value="">Select</option>
                            @foreach($assigns as $row)
                            <option value="{{ $row->emps->id }}" >{{ $row->emps->name }}</option>
                            @endforeach
                            </select>
                            <script>document.getElementById("emp_id").value ="{{ isset($data->emp_id) ? $data->emp_id : '' }}";</script>
                        </div>
            ...
            <div class="row mt-1">
              <div class="col-12 px-0">
                ...
                <button type="button" onclick="assign_emp()" class="btn btn-primary btn-sm btn-add-new" >
                  <i data-feather="plus" class="me-25"></i>
                  <span class="align-middle">Add Emp</span>
                </button>
              </div>
            </div>
          </form>
   ...
   function AddItem(){
        ...
        $.ajax({
               type:'get',
               url: "{{ route('admin.additem') }}?item="+item+"&price="+price+"&qty="+qty+"&apply_id="+apply_id+"&gst="+gst+"&total="+total+"&id="+id, 
...
//Fetch Item Details
   function fetchItem(){
    ...
    $.ajax({
               type:'get',
               url: "{{ route('admin.fetchitem') }}?apply_id="+apply_id, 
               data:'_token = <?php echo csrf_token() ?>',
               success:function(data) {
...
// Delete Item 
  ...
  function assign_emp(){
     var apply_id = document.getElementById("apply_id").value;
        ...
        $.ajax({
               type:'get',
               url: "{{ route('admin.assign_to_to_service') }}?emp_id="+emp_id+"&apply_id="+apply_id, 
               data:'_token = <?php echo csrf_token() ?>',
               success:function(data) {
                  $("#msg").html(data);
                  fetchItem();
